...
static void draw_current_chord()
{
    ...
    uint8_t rootLen = strlen(root_names[tc_app.key.root]);
    uint8_t qualLen = strlen(scale_names[tc_app.key.quality]);
    text_w = (6 + rootLen + qualLen) * 6 - 1;
    uint8_t pos = 64-text_w/2;
    ssd1306_draw_string(&tc_disp, pos, 0, 1, "Key:");
    ssd1306_draw_string(&tc_disp, pos + 25, 0, 1, root_names[tc_app.key.root]);
    ssd1306_draw_string(&tc_disp, pos + 30 + rootLen * 6, 0, 1, scale_names[tc_app.key.quality]);
}
